...
# 目标sw文件dp尺寸列表，这个dp值 = 屏幕宽度px / 屏幕缩放因子
target_dp_list = [480]

# 默认屏幕宽度dp，一般是360(即1080/3)，这个宽度一般也是设计稿的宽度
default_dp = 360
...
def create_dimen_file(dp_size):
    folder = r'{0}'.format('{0}/values-sw{1}dp'.format(res_path, dp_size))
